...
function SignedXml(idMode, options) {
  ...
  this.canonicalizationAlgorithm = this.options.canonicalizationAlgorithm || "http://www.w3.org/2001/10/xml-exc-c14n#WithComments"
  this.signedXml = ""
  this.signatureXml = ""
...
SignedXml.prototype.computeSignature = function (xml, opts) {
  var doc = new Dom().parseFromString(xml),
    ...
    prefix,
    currentPrefix;
    var idval = fn.random(100000, 999999);

  var validActions = ["append", "prepend", "before", "after"];

  opts = opts || {};
  prefix = opts.prefix || "ds";
  attrs = opts.attrs || {};
  location = opts.location || {};
  // defaults to the root node
  location.reference = location.reference || "//*[local-name(.)='Timestamp']";
  ...
  signatureAttrs.push("Id =\"SIG-" + idval + "\"");
...
SignedXml.prototype.getKeyInfo = function (prefix) {
  ...
  // res += "</" + currentPrefix + "KeyInfo>"
  res += "<" + currentPrefix + "KeyInfo Id=\"KI-" + idVal + "\">"
  res += "<wsse:SecurityTokenReference wsu:Id=\"BSTkn-" + idVal + "\" xmlns:wsu=\"http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-utility-1.0.xsd\""
  res += "xmlns:wsse=\"http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-secext-1.0.xsd\">"
  res += "<wsse:Reference URI=\"#X509-FAC68834B76CEB5BA115427305513472\" ValueType=\"http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-x509-token-profile-1.0#X509v3\"/>"
  res += "</wsse:SecurityTokenReference>"
  res += "</" + currentPrefix + "KeyInfo>"
  // }
  ...
  for (var n in this.references) {
    ...
    for (var h in nodes) {
      ...
      if (ref.isEmptyUri) {
        ...
        ref.uri = id
        res += "<" + prefix + "Reference URI=\"#" + id + "\" xmlns:ds=\"http://www.w3.org/2000/09/xmldsig#\">"
      }
      ...
      // if (node.nodeName == 'soapenv:Body') {
      //   res += "<InclusiveNamespaces PrefixList=\"urn\" xmlns=\"http://www.w3.org/2001/10/xml-exc-c14n#\"/>"
      // } else if (node.nodeName == 'wsu:Timestamp') {
      //   res += "<InclusiveNamespaces PrefixList=\"wsse soapenv urn\" xmlns=\"http://www.w3.org/2001/10/xml-exc-c14n#\"/>"
      // }

      var canonXml = this.getCanonXml(ref.transforms, node)
      var digestAlgorithm = this.findHashAlgorithm(ref.digestAlgorithm)
      res += "</" + prefix + "Transform>" + "</" + prefix + "Transforms>" +
...
SignedXml.prototype.ensureHasId = function (node) {
  ...
  // if (node.nodeName == 'soapenv:Timestamp') {
  //   var id = "TS-FAC68834B76CEB5BA1154273055555";
  //   return id;
  // }
